// https://gist.github.com/epoch/b10e6d223581af5520869bd13e918307

// Write a javascript program that you can run with nodejs that randomly generates a startup idea for you. Each time you run it, you should see a random combination for a great startup idea.
...
let randThis = thisArray[Math.floor(Math.random() * thisArray.length)];
let randThat = thatArray[Math.floor(Math.random() * thatArray.length)];

function randomInput(arr) {
...
if (username) {
    console.log(`Hey ${username}, here's an idea for ya!`);
}

// console.log(`${randomInput(thisArray)} for ${randomInput(thatArray)}`)



// dt needs some startup ideas so he can get mega rich. create an object like so:

// let user = {
//   name: 'dt',
//   ideas: []
// }
// use what you've learned to generate 10 ideas for dt and programmatically put it inside the ideas array 
...
let i = 0
while(i < 10) {
    user.ideas.push(`${randomInput(thisArray)} for ${randomInput(thatArray)}`)
    i++
}
